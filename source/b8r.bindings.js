...
const findBindables = element => {
  return findWithin(element, '[data-bind]', true).
    filter(elt => {
...
const findLists = element => {
  return findWithin(element, '[data-list]')
        .filter(elt => {
...
const replaceInBindings = (element, needle, replacement) => {
  ...
  findWithin(element, `[data-bind*="${needle}"],[data-list*="${needle}"]`).forEach(elt => {
...
module.exports = {
  ...
  parseBinding, findLists, findBindables, getBindings, replaceInBindings
};
