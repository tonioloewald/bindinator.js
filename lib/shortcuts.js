...
/**
  ...
  <script>
    const {update} = require('../lib/shortcuts.js');
    update();
    set({
      action: () => alert('yoink!'),
    })
...
let shortcut_targets = b8r.find('[data-shortcut]');

b8r.register('shortcuts', {
  key(evt) {
    const keystroke = b8r.keystroke(evt);
    const matches = shortcut_targets.filter(elt => elt.dataset.shortcut === keystroke);
