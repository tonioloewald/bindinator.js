...
export class HistoryManager {
  constructor(watchPaths, options = {}) {
    ...
    if (!options.historyPath) {
      options.historyPath = '_' + watchPaths[0].split('.').shift() + '_history'
    }
    const {
      historyPath,
      onUndo,
  ...
  reset() {
    ...
    this.undoBuffer.push(
      this.watchPaths.reduce((state, watchPath) => {
        state[watchPath] = b8r.deepClone(b8r.get(watchPath))
        return state
      }, {})
    )
  ...
  observe() {
    this.watchPaths.forEach((path) =>
      b8r.observe(path, `${this.historyPath}.observer`)
    )
  }

  unobserve() {
    this.watchPaths.forEach((path) =>
      b8r.unobserve(path, `${this.historyPath}.observer`)
    )
  }
  ...
  async restore() {
    ...
    this.undoBuffer.forEach((state) => {
      b8r.forEachKey(state, (value, key) =>
        b8r.replace(key, b8r.deepClone(value))
      )
    })
  ...
  async handleChange(pathChanged) {
    if (this.onStateChange && (await this.onStateChange(pathChanged)) === false)
      return
    this.undoBuffer.push({
      [pathChanged]: b8r.deepClone(b8r.get(pathChanged)),
    })
