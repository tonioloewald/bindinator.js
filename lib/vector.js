...
polygon**. The basic idea is if you draw a ray to infinity in any direction from a point p,
the number of times it crosses an edge of a polygon P tells you whether the point is inside
or outside P. If it crosses an **odd** number of edges it is **inside**, otherwise it's **outside**.
...
export const dotProduct = (a, b) =>
  a.map((x, i) => x * b[i]).reduce((sum = 0, x) => sum + x)
...
export const isInPoly = (p, poly, useBoundingBox = true) => {
  ...
  const hits = _poly
    .map((p, i) => xIntercept(p, _poly[i === 0 ? _poly.length - 1 : i - 1]))
