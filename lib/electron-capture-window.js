...
Pass the id of the window you want (e.g. `require.electron_remoted.getCurrentWindow().id`)
...
const captureWindow = (id, callback) => {
  ...
  const [win] = require.electron_remote.BrowserWindow.getAllWindows().filter(w => w.id === id);
  if (win) {
