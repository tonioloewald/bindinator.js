...
A library for loading and saving local files in a web application.
    ...
    import {open, makeDraggableFile, download, fileType} from './path/to/local-files.js'
...
`fileType` sets or removes the type extension of a file.

    const fileName = fileType('text.txt', 'json')     // 'test.json'
    const fileName = fileType('text.txt', '')     // 'test'
~~~~
...
<label>
  ...
  <div 
    ...
    class="drag-file icon-file-empty2"
  ></div>
</div>
...
function download(data, config = saveDefaults) {
  ...
  const data_url = 'data:application/octet-stream;base64,' + btoa(encodeURIComponent(data))
  a.setAttribute('href', data_url)
  a.click()
}

function makeDraggableFile(element, data, config = saveDefaults) {
  const {name, mimeType} = config
