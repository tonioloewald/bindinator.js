...
function download(data, config = saveDefaults) {
  ...
  const dataUrl =
    'data:application/octet-stream;base64,' + btoa(encodeURIComponent(data))
...
function makeDraggableFile(element, data, config = saveDefaults) {
  ...
  element.addEventListener('dragstart', (evt) => {
    ...
    evt.dataTransfer.setData(
      'DownloadURL',
      `application/octet-stream:${fileName}:data:application/octet-stream;base64,${btoa(encodeURIComponent(content))}`
    )
  })
}

export { open, makeDraggableFile, setType, download }
