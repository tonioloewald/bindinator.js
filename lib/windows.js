...
const isParent = () =>
  !(window.opener && window.opener.b8r && window._b8rWindowId)
const childWindows = []
...
class Window {
  constructor(url, options = {}, syncPaths = []) {
    ...
    syncPaths.forEach(
      (path) => observedPaths.indexOf(path) === -1 && observedPaths.push(path)
    )
...
*/
    ...
    if (isElectron) {
      return new Promise((resolve) => {
        this.window.webContents
          .executeJavaScript(`escape(ge.jsON('${path}'))`)
          /* global unescape */
          .then((escapedJson) => {
      ...
      return new Promise((resolve) => resolve(this.window.ge.jsON(path)))
    }
  }

  get(path) {
    return new Promise((resolve) =>
      this.getJSON(path).then((json) => resolve(JSON.parse(json)))
    )
  }
  ...
  setJSON(path, json) {
    ...
    if (isElectron) {
      promise = new Promise((resolve) => {
        this.getJSON(path).then((result) => {
          if (result !== json) {
            ...
            promise = this.window.webContents
              .executeJavaScript(
                `
              se.jsON('${path}', unescape('${escapedJson}'))
            `
              )
              .then(resolve)
          } else {
      ...
      promise = new Promise((resolve) => {
        let result = this.window.ge.jsON(path)
        if (result !== json) {
          result = this.window.se.jsON(path, json)
  ...
  set(path, value) {
    return new Promise((resolve) =>
      this.setJSON(path, JSON.stringify(value)).then(resolve)
    )
  }
}
...
const updateDirtyPaths = b8r.debounce(() => {
  while (_dirtyPathList.length) {
    ...
    if (isParent()) {
      ...
      window.opener.b8r.call(
        'windows-sync-parent.receive',
        window._b8rWindowId,
        path
      )
    }
