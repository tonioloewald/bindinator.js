...
class Window {
  constructor(url, options={}, sync_paths=[]) {
    ...
    if (require.electron) {
      this.window = new require.electron.remote.BrowserWindow(options);
      this.window.loadURL(url);
      const id = require.electron.remote.getCurrentWindow().webContents.id;
...
if (is_parent()) {
  ...
  if (require.electron) {
    const w = require.electron.remote.BrowserWindow.fromId(window._b8r_opener_id);
