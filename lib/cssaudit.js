...
import { makeArray, forEachKey } from '../source/iterators.js'
...
function audit() {
  ...
  makeArray(document.styleSheets).forEach((sheet) => {
    const title = sheet.href || sheet.ownerNode.id
    if (sources[title]) {
    ...
    makeArray(sheet.rules || []).forEach((rule) => {
      ...
      if (!selectors[selector]) {
        selectors[selector] = { count: 0, sources: [] }
      }
  ...
  forEachKey(selectors, (data, selector) => {
    ...
    if (data.count > 1) {
      data.sources.forEach((source) =>
        sources[source].duplicateSelectors.push(selector)
      )
    }
    if (data.missingMatches.length > 0) {
      data.sources.forEach((source) =>
        sources[source].unusedSelectors.push(selector)
      )
    }
  })
  ...
  forEachKey(sources, (data, source) => {
    if (data.unusedSelectors.length || data.duplicateSelectors.length) {
      console.log(
        source,
        'unused: ',
        data.unusedSelectors.join(', '),
        'duplicates: ',
        data.duplicateSelectors.join(', ')
      )
    }
  })
}
