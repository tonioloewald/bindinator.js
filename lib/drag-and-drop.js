...
<script>
  const {draggedElement} = require('lib/drag-and-drop.js');
...
const mark_droppable = types => {
  ...
  elements.forEach(element => {
    const drop_types = element.dataset.drop.split(';');
...
const dnd = {
  ...
  drop (evt) {
    ...
    if (target) {
      const drop_types = target.dataset.drop.split(';');
