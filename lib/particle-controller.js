...
/* global HTMLCanvasElement, requestAnimationFrame */
'use strict'

import b8r from '../source/b8r.js'
import TWEEN from 'https://cdnjs.cloudflare.com/ajax/libs/tween.js/18.6.0/tween.esm.js'
import * as THREE from 'https://cdnjs.cloudflare.com/ajax/libs/three.js/r120/three.module.js'

const particleController = {}
const isApple = ['MacIntel', 'iPod', 'iPhone', 'iPad'].includes(
  navigator.platform
)
    ...
    initParticle(
      {
        ...
        scale,
        fadein:
          particles.length > 1 && idx > 0
            ? 0.1
            : Math.min(fadein, 1 / particles.length),
        fadeout:
          idx + 1 < particles.length
            ? 0.9
            : Math.max(fadeout, 1 - 1 / particles.length),
...
setInterval(
  () => b8r.set('particle-controller.count', scene.children.length),
  500
)

const aspectRatio = window.innerWidth / window.innerHeight
const camera = new THREE.PerspectiveCamera(90, aspectRatio, 1, 5000)
...
Object.assign(particleController, {
  spawnAt: (x, y, glyphs, howmany = 25, scale = 48) => {
    ...
    const projectionDistance =
      camera.position.z / (camera.position.z - vector.z)
