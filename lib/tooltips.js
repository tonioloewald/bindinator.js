...
<button
  style="position: fixed; bottom: 5px; left: 5px; z-index: 10"
  aria-label="
    Do you see any Teletubbies in here? Do you see a slender plastic tag clipped to my shirt with my
    name printed on it? Do you see a little Asian child with a blank expression on his face sitting
    outside on a mechanical helicopter that shakes when you put quarters in it? No? Well, that's what
...
>Out-of-bounds-test</button>
<script>
  const {tooltip} = require('lib/tooltips.js');
  const tip = tooltip();

  tip.style.background = '#007'; // James Bond Blue!
  tip.style.boxShadow = '0 1px 2px rgba(0,0,0,0.5)';
...
const makeStylesheet = require('../source/b8r.makeStylesheet.js');

const tooltip = b8r.create('div');
tooltip.classList.add('tooltips-tooltip');
tooltip.style.position = 'fixed';
tooltip.style.zIndex = 999;
tooltip.style.pointerEvents = 'none';

let tooltip_target = null;
let remove_timeout = null;

b8r.register('tooltip-controller', {
  show(evt) {
    const {target} = evt;
    const text = target.getAttribute('aria-label');
    if (tooltip_target !== target && text) {
  ...
  hide(evt) {
    if (evt.target === tooltip_target) {
      tooltip.style.opacity = 0;
      tooltip_target = null;
      remove_timeout = setTimeout(() => tooltip.remove(), 500);
    }
    return true;
  }
});
