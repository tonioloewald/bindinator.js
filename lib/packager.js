...
const uglify_options = {compress: true, mangle: true};
...
function minify_html(source, uglify=true) {
  return source.replace(/<!--[^]*?-->/gm, '')
               .replace(/(<script.*?>)([^]*)(<\/script>)/gm, (_, start, code, end) => {
                 if (uglify && minify && (! code.match(contains_debugger))) {
                   ...
                   if (minified.err) console.error('minify failed', minified.err);
                   else code = minified.code;
                 }
  ...
  for (const i in module_paths) {
    ...
    if (source) {
      if (uglify && minify && file_filter(module_name) && (! source.match(contains_debugger))) {
        const minified = minify(source, uglify_options);
        if (! minified.error) {
          sources.push([module_name_raw, minified.code]);
        } else {
          console.error('could not minify', module_name_raw, minified.error);
...
async function inline_components (path, component_list, uglify=true) {
  for (const i in component_list) {
    ...
    component_list[i].source = minify_html(source_raw, uglify);
  }
