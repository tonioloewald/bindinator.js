  ...
  rotate: {
    setup: function (settings) {
      ...
      this.panel
        .find('span')
        .append(
          '<label>Angle<input type="range" min="-180" max="180" step="0.5" value="0.0"></label>'
        )
      var self = this
  ...
  text: {
    ...
    apply: function (settings) {
      ...
      if (settings.fillOpacity > 0) {
        ...
        this.cg.fillRect(
          settings.x - 4,
          settings.y - settings.fontSize - 4,
          rect.width + 8,
          settings.fontSize * 1.33 + 8
        )
  ...
  gamma: {
    setup: function (settings) {
      ...
      this.panel
        .find('span')
        .append(
          '<label>Gamma<input type="range" min="0.25" max="2.0" step="0.1" value="1.0"></label>'
        )
      this.panel.find('input').on('change', function () {
        self.refresh()
    ...
    apply: function (settings) {
      var imageData = this.cg.getImageData(
        0,
        0,
        this.canvas.width,
        this.canvas.height
