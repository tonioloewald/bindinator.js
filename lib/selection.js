...
function elementAndOffsetAtOffset(element, offset) {
  ...
  if (element.textContent.length < offset) {
    return { element, offset: element.textContent.length }
  }
...
function getSelection(element) {
  if (
    ...
    return {
      ...
      selectedText: element.value.substr(
  ...
  element.normalize()
  const start =
    selection.anchorOffset + lengthOfTextBefore(selection.anchorNode, element)
  const end =
    selection.focusOffset + lengthOfTextBefore(selection.focusNode, element)
...
function setSelection(editable, selStart = null, selEnd = null) {
  if (selStart === null) {
    selStart = 0
  ...
  if (editable instanceof HTMLInputElement) {
    editable.selectionStart = selStart
    editable.selectionEnd = selEnd
    return
  }
  ...
  {
    const { element, offset } = elementAndOffsetAtOffset(
      editable,
      selEnd || selStart
    )
