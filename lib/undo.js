...
UndoManager leverages `b8r`'s registry to make it easy to implement undo.

Simplest usage:

    const {UndoManager} = await import('path/to/undo.js');
    const undoManager = new UndoManager('some-path', options)

The `undoManager` instance has `undo` and `redo` methods and exposes an `undoBuffer`
and a `redoBuffer` but more importantly, registers all of these at a path that
defaults to `_PATH_history` where PATH is the (first) path the undoManager was told to watch.
(In the preceding example, that would be `_some-path_history`).

It follows that you can implement an **undo** button like this:

    <button
      data-event="click:_some-path_history.undo"
      data-bind="enable_if=_some-path_history.undoBuffer.length"
    >undo</button>

One undoManager can handle state changes across multiple registry paths:

    const undoManager = new UndoManager(['some-path', 'other-path'], {
      historyPath, // path for undoManager's properties
...
The options object allows you to specify a path for the history object and 
provide async callbacks to handle `undo`, `redo`, and user state changes (e.g.
...
<button 
  ...
  data-bind="enabled_if=_undo-example_history.undoBuffer.length"
>undo</button>
<button 
  data-event="click:_undo-example_history.redo"
  data-bind="enabled_if=_undo-example_history.redoBuffer.length"
>redo</button><br>
...
<script>
  ...
  const undoManager = new UndoManager('undo-example', {
    onStateChange: pathChanged => {
      if (undoManager.undoBuffer.length === 0) return
      let current = b8r.get('undo-example.text')
      let previous = b8r.last(undoManager.undoBuffer)['undo-example.text']
      if (! previous) return
      ...
      if (current.length === previous.length) {
        ...
        if (differences === 1) undoManager.undoBuffer.pop();
      }
    }
  })
  set('destroy', () => {
...
b8r.register(path, {foo: 'bar'})
const undoManager = new UndoManager(path)

const {historyPath} = undoManager
Test(() => undoManager.initialState[path].foo, 'state updated').shouldBe('bar')
...
Test(() => b8r.get(`${path}.foo`), 'state updated').shouldBe('lurman')
await b8r.call(`${historyPath}.redo`)
Test(() => b8r.get(`${path}.foo`), 'state updated').shouldBe('romeo + juliet')
await b8r.call(`${historyPath}.undo`)
await b8r.call(`${historyPath}.undo`)
await b8r.call(`${historyPath}.redo`)
Test(() => b8r.get(`${path}.foo`), 'state updated').shouldBe('lurman')
...
export class UndoManager {
  constructor (watchPaths, options={}) {
    ...
    this.initialState = watchPaths.reduce(
      (state, watchPath) => {
  ...
  async restore () {
    this.unobserve()
    const state = b8r.last(b8r.get(`${this.historyPath}.undoBuffer`))
     || deepClone(this.initialState)
  ...
  undo = async () => {
    ...
    if (this.undoBuffer.length) {
      ...
      await this.restore(this.initialState);
    }
    return true
  }
