...
For convenience when working with Electron you can use:

    require.globalRequire // electron's require
    require.electron // require.global_require('electron')

For quick interaction with electron, you can thus use:

    require.electron.remote

E.g.

    require.electron.remote.getCurrentWindow() // BrowserWindow instance

In cases where in a vanilla Electron app you'd write:
...
Aside: given that b8r's `require` doesn't do magical conversions between module names
and paths, it would be pretty easy to 'automagically' use globalRequire if a
source file doesn't end in `.js`, so require('fs') would _just work_.
*/
...
(function(global) {
  ...
  function define(module_name, source_code) {
    ...
    if (! modules[module_name]) {
      const module = module_name.substr(-1) === '~' ? {} : {exports:{}};
      const factory = typeof source_code === 'function'
                      ? source_code
                      : new Function('module', 'require', `${source_code}\n//# sourceURL=${module_name}`);
      /* jshint evil:false */
  ...
  function _require(module_name) {
    ...
    if (! modules[module_name]) {
      if (! _files[module_name]) {
        ...
        if (autoPreload_enabled) _savePreloadData();
        if (syncLoadHandler) syncLoadHandler();
  ...
  function _asyncLoadFile(module_name) {
  ...
  function _file(module_name) {
    ...
    if (!_file_promises[module_name]) {
      _file_promises[module_name] = _asyncLoadFile(module_name);
    }
  ...
  const _preloadData = (return_native_data=false) => {
    let available = [];
    const module_list = Object.keys(modules);
    ...
    while(module_list.filter(not_available).length) {
      const wave = module_list.filter(not_available).filter(no_deps).sort();
      waves.push(wave);
      if(wave.length === 0) {
    ...
    return return_native_data ? waves : JSON.stringify(waves, false, 2);
  };
  ...
  Object.assign(require_utils, {
    ...
    viaTag: _viaTag,
    electron: ! global.nw && global.process && _global_require('electron'),
    globalRequire: _global_require,
    modules: () => modules,
    onSyncLoad: handler => {
      if (syncLoadHandler) console.warn('replaced existing syncLoadHandler');
      syncLoadHandler = handler;
    }
  });
