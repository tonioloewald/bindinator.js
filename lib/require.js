...
(function(global) {
  ...
  function _lazy(module_name) {
    ...
    if (!_lazy_module_promises[module_name]) {
      _lazy_module_promises[module_name] = new Promise(function(resolve, reject) {
        if (!modules[module_name]) {
          let request = new XMLHttpRequest();
          request.open('GET', module_name.substr(-1) !== '~' ? module_name : module_name.slice(0, -1), true);
          request.onload = function(data) {
            if (request.readyState === 4) {
              if (request.status === 200) {
                resolve(define(module_name, request.responseText).exports);
              } else {
                reject(request, data);
              }
            }
