...
(function(global) {
  ...
  const _savePreloadData = () => {
    ...
    savePreloadDataTimeout = setTimeout(() => {
      ...
      localStorage.setItem('_require_preloadData', _preloadData(false));
    ...
    function preload(waves) {
      if (waves.length) {
        ...
        return Promise.all(wave.map(_lazy))
            ...
            .catch(() => {
              console.error('auto preload failed');
              localStorage.removeItem('_require_preloadData');
  ...
  const _autoPreload = (save_preload_data_throttle_ms) => {
    ...
    return new Promise(resolve => {
      const waves = localStorage.getItem('_require_preloadData');
      if (waves) {
  ...
  global.require.autoPreload = _autoPreload;
