...
> `require` could simply do all this for you, so you could just do something like:
>
>     require.autoPreload().then(...);
>
> Implementation would be straightforward. If `autoPreload()` has been called, then
> any synchronous load would cause `preloadData()` to be stored in `localStorage`
> (throttled). `autoPreload()` itself looks for previously saved preload data returns
...
> more, each client's loading pattern is built around their specific usage. If
> they never load a big chunk of your code, it never gets preloaded. If they
> use Czech and Vietnamese localizations, they have exactly those preloaded.
*/
/* global console */
