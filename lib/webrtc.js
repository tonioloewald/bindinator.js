...
const update = () => {
  navigator.mediaDevices.enumerateDevices().then(
    devices => {
      ...
      if (settings.videoinput && !sources.videoinput.find(device => device.deviceId === settings.videoinput)) {
        settings.videoinput = null;
      }
      if (settings.audioinput && !sources.audioinput.find(device => device.deviceId === settings.audioinput)) {
