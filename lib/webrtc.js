...
const listeners = []
const { desktopCapturer } = isElectron
  ? require('electron')
  : { desktopCapturer: false }
...
const updateMediaDevices = (desktopSources) => {
  navigator.mediaDevices.enumerateDevices().then((devices) => {
    ...
    if (
      settings.videoinput &&
      sources.videoinput &&
      !sources.videoinput.find(
        (device) => device.deviceId === settings.videoinput
      )
    ) {
    ...
    if (
      settings.audioinput &&
      sources.audioinput &&
      !sources.audioinput.find(
        (device) => device.deviceId === settings.audioinput
      )
    ) {
...
export { update, addListener, removeListener }
