...
export function tag2Lines(elt, indent = 0) {
  ...
  const attributeText = attributes.length
    ...
    : ''
  const onlySimpleAttributes = !attributes.find(({ key }) =>
    BINDING_ATTRIBUTES.includes(key)
  )
...
export function node2Lines(node, indent = 0) {
  ...
  const childNodes = [...node.childNodes].filter(
    (node) => !(node instanceof Text) || node.textContent
  )
  if (
    childNodes.length === 0 &&
    tagLines.length === 1 &&
    tagLines[0].length + tagName.length < 77
  ) {
  ...
  return [...tagLines, ...childLines, spaces(indent) + `</${tagName}>`].flat()
}
...
export function childNodes2Text(node, indent = 0) {
  return [...node.childNodes]
    .filter((child) => !(child instanceof Text) || child.textContent.trim())
    .map((child) => node2Text(child, indent))
    .join('\n')
}
