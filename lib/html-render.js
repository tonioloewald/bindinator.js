...
<div class="bar">
  ...
  <button data-event="click:_component_.outerHTML">outerHTML</button>
...
<script>
  import('../web-components/code-editor.js').then(() => {
    ...
    data.outerHTML = async () => {
      const {view} = await b8r.component(get().component)
      textarea.value = view.outerHTML
      set({title: `view.outerHTML of ${get('component')} component`})
    }
...
export function tag2Lines(elt, indent = 0) {
  ...
  let attributeText = attributes.length ?
                        ' ' + attributes
...
export function node2Lines (node, indent = 0) {
  ...
  return [
    ...tagLines,
    ...childNodes.map(c => node2Lines(c, indent + 1)),
