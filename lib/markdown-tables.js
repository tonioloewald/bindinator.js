...
simple implementation to meet my own needs which is compatible (to some extent)
with the most popular table extension to markdown.
...
**Indentation is preseted** so a table whose rows are consistently indented
will be rendered as a single line of inline html with the same indentaton.

A table is inferred from a series of table rows with the same indent and no empty lines
separating sequential rows.

Only a tiny subset of Markdown syntax is supported inside table cells, namely
the **bold**, _italic_, and `code` inline syntax.

```
export default {
  css: `textarea { width: 100%; min-height: 160px; }`,
  html: `
    <textarea data-bind="value=_component_.source"></textarea>
    <div data-bind="_component_.render=_component_.source"></div>
  `,
  initialValue: {
    source: `
||foo ||bar
...
| and \`code\` | work
        `.trim(),
    render: async(elt, source) => {
      const {renderTables} = await import('../lib/markdown-tables.js')
      elt.innerHTML = renderTables(source)
    }
  },
}
```
...
Test(
    () => renderTable([{indent: '> ', content: '||foo ||bar'},{indent: '> ', content: '|17 |42'}]),
...
| Can have header cells anywhere | Heading rows only
| Cannot change text alignment | Can change text

### Pipe Characters inside Tables

Finally, to insert a "pipe" character inside a cell, you should use the HTML escape code
for a pipe, i.e `&#124;`
*/

export const cellContentRenderer = (text) =>
  text
...
export const renderTable = (rows) =>
  `${rows[0].indent}<table>${rows.map(renderRow).join('')}</table>`
export const renderTables = (source) => {
  ...
  rawLines.forEach((line, idx) => {
    ...
    if (
      rows.length &&
      (indent !== rows[0].indent || !content || !content.trim())
    ) {
