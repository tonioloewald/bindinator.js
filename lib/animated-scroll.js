...
# animated_scroll
    ...
    animated_scroll(scrolling_element, desired_scrolltop, desired_scrollleft=0);

Perform a snappy animated scroll on the specified element to the specified position.

    animated_scroll.visible(elt);

`animated_scroll.visible(elt)` will return `true` if `elt` is wholly visible (i.e. not clipped
by its direct parentElement).

    animated_scroll.into_view(elt);

`animated_scroll.into_view(elt)` will check if the element is visible and not it will scroll
it into view using animated_scroll.
...
<script>
  const {into_view} = require('../lib/animated-scroll.js');
  const show = evt => {
...
const animated_scroll = (elt, top, left=0) => {
...
animated_scroll.into_view = elt => {
  if (animated_scroll.visible(elt)) return;

  const parent = elt.parentElement;
  const max_top = elt.offsetTop + elt.offsetHeight - parent.clientHeight;
  const top = parent.scrollTop > elt.offsetTop
              ? elt.offsetTop
              : parent.scrollTop < max_top
              ? max_top
              : parent.scrollTop;
  const max_left = elt.offsetLeft + elt.offsetWidth - parent.clientWidth;
  const left = parent.scrollLeft > elt.offsetLeft
               ? elt.offsetLeft
               : parent.scrollLeft < max_left
               ? max_left
               : parent.scrollLeft;

  animated_scroll(parent, top, left);
};

module.exports = animated_scroll;
