...
b8r.register('touch-game-controller', {
  ...
  stick_move (evt) {
    const id = evt.type === 'touchstart' ? evt.changedTouches[0]: 'mousedown';
    let stick = b8r.findValue(sticks, stick => stick.id === id);
  ...
  button_down(evt, element) {
    ...
    if (stick) {
      ...
      if (button.element.dataset.radioset) {
        ...
        const button_path = `touch-game-controller.buttons.${button.title}`;
        b8r.set(`${button_path}.toggle`, ! b8r.get(`${button_path}.toggle`));
      }
      console.log(button.title, button.active, button.toggle ? 'on' : 'off');
      delete button.id;
    } else {
