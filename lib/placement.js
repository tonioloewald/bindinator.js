...
(loading placement.js registers `placement-controller`).
    ...
    above(element, min_z=1);

Set the element's z-index to be greater than all siblings, and at least `min_z`.

Because of the weird way z-index interacts with static positioning, the only
way to truly figure out the minimum greatest z-index would be to walk most of
the DOM.
