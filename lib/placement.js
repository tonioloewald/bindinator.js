...
import { register, set, get, remove } from '../source/registry.js'
import { on } from '../source/events.js'
import { onAny, offAny } from '../source/anyEvent.js'
...
const above = (element) => {
  const siblings = [].slice
    .apply(element.parentElement.children)
    .filter((e) => e !== element)
  const z = siblings.length ? siblings.map(getZIndex).sort().pop() + 1 : 1
