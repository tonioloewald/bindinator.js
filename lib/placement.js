...
const {register, set, get, remove} = require('../source/b8r.registry.js');
const {on} = require('../source/b8r.events.js');
const {onAny, offAny} = require('../source/b8r.anyEvent.js');

const get_z_index = elt => {
  const style = getComputedStyle(elt);
  if (style.position === 'static' && elt.children.length) {
    return [].slice.apply(elt.children).map(get_z_index).sort().pop();
  } else {
...
module.exports = {float, above, remove_placement};
