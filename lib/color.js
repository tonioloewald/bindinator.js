...
require.lazy('./css-color-names.js').then(data => _cssColors = data);

const cssColors = () => {
  if(!_cssColors) {
    _cssColors = require('./css-color-names.js');
  }
  return _cssColors;
};
...
`color_distance` is the cartesian distance between the two colors multiplied by the
...
~~~~
const {parse} = require('./lib/color.js');
const black = parse('black');
const white = parse('white');
const red = parse('#f00');
const transparent_white = parse('rgba(255,255,255,0)');
Test(() => white.difference(black).color_distance).shouldBe(1);
Test(() => red.difference(black).color_distance).shouldBe(Math.sqrt(1/3));
Test(() => red.difference(white).color_distance).shouldBe(Math.sqrt(2/3));
console.log(black.difference(white))
...
*/
  difference(other_color) {
    const r = Math.abs(this.r - other_color.r) / 255;
    const g = Math.abs(this.g - other_color.g) / 255;
    const b = Math.abs(this.b - other_color.b) / 255;
