...
A library for comparing, manipulating, converting colors between representations and performing math on colors.
...
Returns an {h, s, v, a} object representation of the color. `h` is 0-360°.
...
*/
  constructor(r, g, b, a) {
    ...
    this.a = a !== undefined ? clamp(0, a, 1) : 1
  }
...
function parse(colorSpec) {
  ...
  const [, rgb, , rgba, , html6, html3, name] = colorSpec.match(
    /(rgb\(\d+(,\d+){2}\))|(rgba\(\d+(,\d+){2},[\d.]+\))|(#[0-9a-fA-F]{6})|(#[0-9a-fA-F]{3})|(\w+)/
  )
  var r = 0
  var g = 0
  var b = 0
  ...
  if (rgb) {
    ...
    if (name && cssColors()[name]) {
      const namedHex = cssColors()[name]
      ;[r, g, b] = namedHex.match(/[\da-fA-F]{2}/g).map((h) => parseInt(h, 16))
    } else {
