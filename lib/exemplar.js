...
`numType` will return true for numbers and false for anything other than a number.
...
Creates an exemplar for testing a sequence of arguments for childproofing a function.
...
Creates an exemplar that accepts an array of elements matching one of the provided examples.

    exemplar.regex(regular_expression)

Creates an exemplar that accepts strings that match the regular expression provided.
...
creates an exemplar that only accepts object with a property `foo` that is a number.
...
const geoType2 = objType.properties({name: 'fred', position:objType.properties({x: 10, y: 100})});
...
Produces a *childproofed* function that will throw if passed arguments other that it expects
or if it returns a value other than expected (returns is optional — if omitted, the function
is expected *not* to return a value).

expects or returns are treated as type references if a string if passed, and treated as 
*/
...
const specifiers = {
  ...
  nonempty: function(){
    const test = this;
    return exemplar(subject => test(subject) && types.number(subject.length) && subject.length > 0).describe('non empty ' + test.description);
  },
  nonzero: function() {
    const test = this;
  ...
  properties: function(object) {
    ...
    return exemplar(subject => test(subject) && checkProps(subject)).describe(description);
  },
...
Test(() => exemplar.types.stringArray(['hello', 17, 'world'])).shouldBe(false);
...
Test(() => exemplar.types.element(document.createElement('div'))).shouldBe(true);
Test(() => exemplar.types.nodeList(document.querySelectorAll('input'))).shouldBe(true);
~~~~
*/
...
var types = {
  ...
  number: exemplar(0),
  int: exemplar(subject => !(subject%1)).describe('int'),
  ...
  array: exemplar([]),
  arrayish: exemplar({}).and(subject => typeof subject.length === 'number').describe('arrayish (has length)'),
  ...
  element: exemplar(subject => subject instanceof HTMLElement),
