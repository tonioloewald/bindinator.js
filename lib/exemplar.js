...
const specifiers = {
  ...
  optional: function () {
    const test = this
    return exemplar(
      (subject) => test(subject) || types.undefined(subject)
    ).describe(test.description + ' (optional)')
  },
  nullable: function () {
    const test = this
    return exemplar((subject) => test(subject) || types.null(subject)).describe(
      test.description + ' OR null'
    )
  },
  nonempty: function () {
    const test = this
    return exemplar(
      (subject) =>
        test(subject) && types.number(subject.length) && subject.length > 0
    ).describe('non empty ' + test.description)
  },
  nonzero: function () {
    const test = this
    return exemplar((subject) => test(subject) && subject !== 0).describe(
      'non-zero ' + test.description
    )
  },
  positive: function () {
    const test = this
  ...
  among: function (options) {
    const test = this
    return exemplar(
      (subject) => test(subject) && options.indexOf(subject) > -1
    ).describe(`(${options.join('|')})`)
  },
  and: function (otherTest) {
    const test = this
    return exemplar((subject) => test(subject) && otherTest(subject)).describe(
      `${test.description} and ${otherTest.description}`
    )
  },
  or: function (otherTest) {
    const test = this
    return exemplar((subject) => test(subject) || otherTest(subject)).describe(
      `${test.description} or ${otherTest.description}`
    )
  },
  not: function () {
    const test = this
    return exemplar((subject) => !test(subject)).describe(
  ...
  properties: function (object, recursivelyCheckProperties = false) {
    ...
    return exemplar((subject) => test(subject) && checkProps(subject)).describe(
      description
    )
  },
...
function exemplar(example, checkProperties = false) {
  ...
  if (checkProperties && type === 'object')
    test = test.properties(example, true)
  return test
}
...
exemplar.heteroArray = function (...args) {
  ...
  return exemplar(
    (args) =>
      ...
      (args.length === 0 ||
        args
          .map((arg) => tests.map((test) => test(arg)).reduce(OR))
          .reduce(AND)),
...
const types = {
  ...
  function: exemplar((subject) => subject instanceof Function).describe(
    'function'
  ),
  ...
  noarguments: exemplar((subject) => subject.length === 0).describe(
    'no arguments'
  ),
...
const childproof = _childproof // (_childproof,
