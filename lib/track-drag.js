...
const _move = (evt, dragEnded = false) => {
  ...
  const touch = isMouseEvent
    ? null
    : [...evt.changedTouches].find((c) => c.identifier === touchIdentifier)
...
/*
  We don't check for the zIndex getting too large because the sun will
  explode long long before that happens.
*/

export const trackDrag = (initialEvent, origX, origY, callback) => {
  const isMouseEvent = initialEvent.type === 'mousedown'
  _moveEventDiv.style.zIndex = findHighestZ() + 1
  const { pageX, pageY } = isMouseEvent
    ? initialEvent
    : initialEvent.changedTouches[0]
  _moveState = {
    callback,
    pageX,
    pageY,
    ...
    lastX: origX,
    lastY: origY,
    touchIdentifier: isMouseEvent
      ? null
      : initialEvent.changedTouches[0].identifier,
  }
  if (isMouseEvent) {
