...
Here's an example (the service is at `./screencap/url/to/capture`)

#### Screen Cap
![puppetteer screenshot](/screencap/?body=components/richtext)

#### Forced Dark Mode
![puppetteer screenshot](/screencap/?body=components/richtext&darkmode=true)

#### Forced Light Mode
![puppetteer screenshot](/screencap/?body=components/richtext&darkmode=false)

### TODO

- `GET` `/darkmode/` endpoint -- returns darkmode state
- `POST` `/darkmode/` `{dark: true|false}` -- set darkmode
- `GET` `/screencap/dark|light/url` -- renders urls
- `GET` `/screencap/` `{dark: true|false, url: string, id: string}` -- compares rendered url to golden version (if present), returns 'match', 'fail', or 'new'
- `POST` `/screencap/` `{dark: true|false, url: string, id: string}` -- creates new golden screencap
...
export class Test {
  ...
  shouldBe (value, epsilon = 0) {
    if (this._test) {
      throw new Error('cannot set a different expectation')
    }
    this._expected = `expected ${value}`
    this._test = x => epsilon ? x - epsilon < value && x + epsilon > value : x === value
  }
