...
export class Test {
  ...
  shouldBe(value, epsilon = false) {
    ...
    this._test = (x) =>
      epsilon > 0 ? x - epsilon < value && x + epsilon > value : x === value
    return this.drained
  }
  ...
  success(result) {
    this.status = 'success'
    this.report(
      `${this.description} ${this.testedRelationship} ${quote(result)}`,
      'success'
    )
...
export const waitMs = (durationMs = 1000, value) =>
  new Promise((resolve) => {
    setTimeout(() => resolve(value), durationMs)
  })

export const throwAfterMs = async (
  errorToThrow = new Error('something bad happened'),
  delayMs = 100
) => {
...
export function trigger(eventType, target) {
  ...
  targets.forEach((target) => {
    target.dispatchEvent(new Event(eventType, { bubbles: true, view: window }))
  })
}
