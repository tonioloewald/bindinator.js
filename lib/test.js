...
export class Test {
  ...
  shouldBe (value, epsilon = false) {
    ...
    this._expected = epsilon > 0 ? `expected ${value}&plusmn;${epsilon} ` : `expected ${value}`
    this.testedRelationship = epsilon > 0 ? '~=' : '=='
  ...
  failure (result) {
    ...
    this.report(`!(${this.description} ${this.testedRelationship} ${quote(result)})${expectation}`, 'failure')
  }
