    ...
    rows.forEach((row, row_index) => {
      ...
      row.forEach(elt => {
        ...
        const css = {
          ...
          height: row_height.toFixed(1) + '%',
          left: left.toFixed(1) + '%',
          top: (row_height * row_index).toFixed(1) + '%',
        };
        ...
        if (letterbox_threshold && _distortion > letterbox_threshold) {
          if (cell_aspect_ratio < target_aspect_ratio) {
            ...
            css.height = (row_height / _distortion).toFixed(1) + '%';
            css.top = (row_height * row_index + (row_height - row_height / _distortion) * 0.5).toFixed(1) + '%';
          } else {
            // target is taller than the cell, so shrink it and push it right
            css.width = (width / _distortion).toFixed(1) + '%';
            css.left = (left + (width - width / _distortion) * 0.5).toFixed(1) + '%';
          }
        }
