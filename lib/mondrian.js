...
If you add `transition: var(--hover-transition)` (say) then elements will transition between layouts.
...
const elementAspectRatio = (element) => {
  if (!element.matches(sizeTargetSelector)) {
    element =
      [...element.querySelectorAll(sizeTargetSelector)].find(
        (elt) => getComputedStyle(elt).display !== 'none'
      ) || element
  }
  let w, h
...
function arrange(
  ...
  while (rowCount * colCount < itemCount) {
    ...
    var distortionWithNewRow = distortion(
      (rowCount + 1) / colCount / targetAspectRatio
    )
    var distortionWithNewCol = distortion(
      rowCount / (colCount + 1) / targetAspectRatio
    )
...
function arrangeWithFocus(
  ...
  const focusIdx = elements.findIndex((elt) =>
    elt.classList.contains('mondrian-focus')
  )
  ...
  if (elements.length > 1 && focusIdx > -1) {
    ...
    if (
      distortion(
        (focusAspectRatio / containerAspectRatio) * (focusBias / 100)
      ) <
