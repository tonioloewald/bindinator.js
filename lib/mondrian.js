  ...
  arrange(elements, aspect_ratio, letterbox_threshold) {
    ...
    rows.forEach((row, row_index) => {
      ...
      row.forEach(elt => {
        ...
        const mondrian_size_target = elt.matches(size_target_selector) ? elt : elt.querySelector(size_target_selector);
        if (letterbox_threshold && mondrian_size_target) {
          const target_aspect_ratio = element_aspect_ratio(mondrian_size_target);
          const _distortion = distortion(cell_aspect_ratio / target_aspect_ratio);
          // console.log(_distortion > letterbox_threshold, {cell_aspect_ratio, target_aspect_ratio, _distortion});
