...
const getDimensions = elt => {
  if (elt.dataset.biggridItemSize) {
    ...
    delete clone.dataset.list;
    clone.style.display = '';
    clone.style.position = 'absolute';
    clone.style.visibility = 'hidden';
    elt.parentElement.insertBefore(clone, elt);
    const style = window.getComputedStyle(clone);
    const {marginLeft, marginRight, marginTop, marginBottom} = style;
    const marginWidth = toInt(marginLeft) + toInt(marginRight);
    const marginHeight = toInt(marginTop) + toInt(marginBottom);
    const dimensions = {
      width: clone.offsetWidth + marginWidth,
      height: clone.offsetHeight + marginHeight,
    };
    clone.remove();
    return dimensions;
  }
...
const slice = (list, target, single_column=false) => {
  ...
  const signature = b8r.elementSignature(container);
  b8r.logStart('biggrid', 'slice ' + signature);
  ...
  const new_row_size = single_column ? 1 : Math.max(1, Math.floor(w / slice.item.width));
  let scrollTop = container.scrollTop;
  let resizing = slice.row_size !== new_row_size;
  let centralIndex, centralRow;
  if (resizing) {
    if (slice.row_size) {
      centralIndex = list.indexOf(b8r.getListInstance(document.activeElement));
      if (centralIndex === -1) {
        centralIndex = Math.floor((slice.first_row + slice.visible_rows * 0.5) * slice.row_size);
      }
    } else {
      resizing = false;
    }
    slice.row_size = new_row_size;
  }
  slice.rows = Math.ceil(list.length / slice.row_size);
  slice.visible_rows = Math.min(Math.ceil(h / slice.item.height + 1), slice.rows);
  if (resizing) {
    centralRow = Math.floor(centralIndex / slice.row_size);
    // workaround for chrome bug where chrome will not set scrollTop correctly
    scrollTop = clamp(
      centralRow * slice.item.height + (slice.item.height - h) * 0.5,
      0,
      (slice.rows - slice.visible_rows) * slice.item.height
    );
    container.scrollTop = scrollTop;
    // console.log('resizing', centralIndex, centralRow, scrollTop);
  }
  slice.first_row = clamp(
    Math.floor(scrollTop / slice.item.height),
    0,
  ...
  if (resizing) {
    // again, we're trying to force chrome to scroll correctly
    container.scrollTop = scrollTop;
  }

  // console.log(slice.item, slice);

  b8r.logEnd('biggrid', 'slice ' + signature);
