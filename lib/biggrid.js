...
`biggrid.js` provides a **controller** for virtual grids of fixed-size elements

> Also see [biggrid-test](./?source=biggrid-test.component.html) and
> [data-table](./?source=components/data-table.component.js) to see biggrid
> in action.
...
If you are scrolling a table with a repeating pattern of row colors using :nth-child selectors,
`modulus` will constrain the first row of the slice to be a multiple of modulus (and increase
the size of the slice by modulus) to allow stable `nth-child` offsets. E.g. if your table
rows have a three row pattern, such as light, light, dark, you'd pass 4.
...
const getDimensions = (elt) => {
  ...
  if (eltWithSize) {
    const [width, height] = eltWithSize.dataset.biggridItemSize
      .split(',')
      .map(toInt)
    return { width, height }
  } else {
...
const update = (_evt, target) => {
  ...
  if (!updater) {
    updater = {
      target,
      update: b8r.throttleAndDebounce(() => _update(target), 30),
    }
...
const slice = (list, target, singleColumn = false, modulus = false) => {
  ...
  const [padWidth, padHeight] = (container.dataset.biggridPadding || '0,0')
    .split(',')
    .map(toInt)
  ...
  if (!useExistingSlice) {
    slice.rowSize = singleColumn
      ? 1
      : Math.max(1, Math.floor(w / slice.item.width))
    slice.rows = Math.ceil(list.length / slice.rowSize)
    slice.visibleRows = Math.min(
      Math.ceil(h / slice.item.height + 1),
      slice.rows
    )
...
export { biggrid, update, slice, scrollToIndex, getDimensions }
