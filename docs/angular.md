...
- write templates in Angular's template language (they say it's HTML
  but this is _just not true_. `*ngFor` is not a valid attribute.)
...
to learn even more languages (_nothing will save you_ from needing to
...
But as we have seen `b8r` is actually _more compact and
expressive_, and no less platform-abstracting than React or Angular
(or Cappucino).
...
new toy programming language _and_ a templating language. And a pretty nasty
...
> By the way, I would _love_ to make `b8r`'s syntax cleaner
...
> I choose _technical_ simplicity and _correctness_.
>
...
> complex, but my goal would be to make the syntax _simpler_ and _cleaner_.

The `b8r` example is simply HTML because `b8r` templates are just HTML. Really.

It's going to be parsed and rendered by the browser using its _native_ HTML
...
a few visits to the documentation to understand _how to write_ some of the
...
> interesting.)
>
> Thinking about this did inspire me to finally implement _implicit
> method bindings_, so you can write
...
- `products` is in exactly _one_ place in the registry. You don't
...
the share method already exist. They're _just_ the "template" piece.

To put it even more starkly: there is nothing code-like in the `b8r`
template (e.g. `share()` or `let product of products`) yet it does _everything
it has to_. The Angular template has code embedded in the HTML and yet
...
A simple component can easily end up being _four different source files_.
...
latter is _not_ designed to live inside and rely on a complex build system
...
(If this seems _just like_ how you'd make a component in a javascript
...
Want to do the same thing in Angular? Well you _could_ try to wrap your Angular component as
...
swaps _statically defined_ elements.
...
This manifests in HTML comments appearing _everywhere_ in the DOM as placeholders.
...
and explanation than the React _To Do_ tutorial (which at least does something).
